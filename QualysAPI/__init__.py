...
class QualysAPI:
    ...
    def makeCall(self, url, payload=None, headers: dict = None, json_body: bool = False, retryCount=0):
        if payload is None:
            payload = ''
        ...
        # If this is a JSON request, we have to add the payload to via json instead of data
        if json_body:
            r = requests.Request('POST', url, json=payload, headers=rheaders)
        else:
